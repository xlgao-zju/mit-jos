...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	len = (size_t)ROUNDUP(va + len, PGSIZE);
	for(;(size_t)va < len; va += PGSIZE){
		if(!(pp = page_alloc(0)))
			panic("region_alloc:Page_alloc fails!");
		i = page_insert(e->env_pgdir, pp, va, PTE_U | PTE_W);
		if(i)
			panic("region_alloc:Page_insert fails!--%e", i);
	}
}
...
load_icode(struct Env *e, uint8_t *binary, size_t size)
{
	...
	prog = (struct Elf *)binary;
	if (prog->e_magic != ELF_MAGIC)
		panic("The binary is not user progerm!");
